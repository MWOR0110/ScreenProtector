...
public class ScreenProtector extends javax.swing.JFrame implements Runnable{
	...
	public void run() {
		...
		SoundEffect Sound = new SoundEffect("/sounds/9mm-pistol-shoot-short-reverb-7152.wav",true);
		...
		for(int i = 0;i < 30; i++) {
			shape.add(new Rectangle(colorGenerate(),
									ran.nextInt(getWidth()-300),
									ran.nextInt(getHeight()-300),
									ran.nextInt(80)+10,
									ran.nextInt(80)+10)) ;
			shape.add(new Circle(colorGenerate(),
								ran.nextInt(getWidth()-300),
								ran.nextInt(getHeight()-300),
								ran.nextInt(80)+10));
			shape.add(new Square(colorGenerate(),
								ran.nextInt(getWidth()-300),
								ran.nextInt(getHeight()-300),
								ran.nextInt(80)+10));
			shape.add(new Elipse(colorGenerate(),
								ran.nextInt(getWidth()-300),
								ran.nextInt(getHeight()-300),
								ran.nextInt(80)+10,
								ran.nextInt(80)+10));
		...
		Image img = new Image("/images/SENAI_logo.png",500,500,"/sounds/impact.wav",false);
		img.scale(0.5);
		...
		while(true) {
			...
			for(Shape i : shape) {
				i.draw(g);
				i.move();
				i.changeDir(0, getWidth(), 0, getHeight());
			}
			
			img.draw(g);
			img.getColision(getWidth(), getHeight());
	      ...
	      try {
	        Thread.sleep(10);//https:academy.oracle.com/en/oa-web-overview.html
